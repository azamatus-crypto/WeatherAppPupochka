package com.base.weatherapppupochka;
...
public class MainActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<JSONObject> {
    ...
    private CityAdapters cityAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        recyclerViewWeather = findViewById(R.id.recyclerViewWeatherchik);
        switchCities = findViewById(R.id.switchCities);
        scrollView = findViewById(R.id.scrolviewmain);
        ...
        cityAdapter = new CityAdapters(new CityAdapters.OnCityClickListener() {
            @Override
    ...
    public void onLoadFinished(@NonNull Loader<JSONObject> loader, JSONObject jsonObject) {
        if (jsonObject == null) {
            Toast.makeText(this, R.string.warning_slow_internet, Toast.LENGTH_SHORT).show();
        }
    ...
    private void setCurrentWeather(JSONObject jsonObject) {
        Weatherentr weatherEntr;
        if (todayWeather) {
            weatherEntr = JSONutils.getWeatherEntryFromJSON(jsonObject);
        } else {
            weatherEntr = JSONutils.getWeatherEntryForTomorrow(jsonObject);
        }
        if (weatherEntr != null) {
            textViewCity.setText(nameOfCity);
            String humidity = (int) weatherEntr.getHumidity() + "%";
            textViewHumidity.setText(humidity);
            String pressure = (int) weatherEntr.getPressure() + "мм";
            textViewPressure.setText(pressure);
            String wind = String.format("%s м/с, %s", (float) weatherEntr.getWindSpeed(), SimpJson.translate(weatherEntr.getWindDirectory()));
            textViewWind.setText(wind);
            String temperature = (int) weatherEntr.getTemp() + "°";
            textViewTemp.setText(temperature);
            textViewWeatherDescription.setText(SimpJson.translate(weatherEntr.getCondition()));
            String imageUri = weatherEntr.getIcon();
            Glide.with(this).load(Uri.parse(imageUri)).into(imageViewWeatherIcon);
    ...
    private void setForecastWeather(JSONObject jsonObject, String tag) {
        String tagToday = getResources().getString(R.string.tag_today);
        String tagTomorrow = getResources().getString(R.string.tag_tomorrow);
        ArrayList<Weatherentr> weatherEntrie;
        if (tag.equals(tagToday)) {
            showTodayWeather();
            weatherEntrie = JSONutils.getForecastToday(jsonObject);
        } else if (tag.equals(tagTomorrow)) {
            showTomorrowWeather();
            weatherEntrie = JSONutils.getForecastTomorrow(jsonObject);
        } else {
            showTodayWeather();
            weatherEntrie = JSONutils.getForecastForThreeDays(jsonObject);
        }
        weatherAdapter.setWeatherEntries(weatherEntrie);
